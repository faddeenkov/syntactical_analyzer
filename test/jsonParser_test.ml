...
let tests = "test suite for JSONParser" >::: [
  ...
  "query with and" >:: (fun _ -> assert_equal query2 (parse_json_file "test/test2.json"));
  "function-returns query" >:: (fun _ -> assert_equal query3 (parse_json_file "test/test3.json"));
  "bad query" >:: (fun _ -> assert_raises (SyntaxError("Wrong syntax: unexpected input for structure")) (fun _ -> (parse_json_file "test/test4.json")))
]
