...
let resolve_query_datatype_decl_none_none query cilfile = 
...
match query.f with Decl_f -> if ((query.str = None_s) && (query.lim = None_c)) then (resolve_query_datatype_decl_none_none query cilfile) else (Printf.printf "Not supported yet.\n"; ("", loc_default, "", -1)::[])
                | _ -> Printf.printf "Not supported yet.\n"; ("", loc_default, "", -1)::[]
