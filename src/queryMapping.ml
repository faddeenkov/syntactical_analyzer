...
match list with (name2, loc2, typ2, id2)::xs -> if (String.compare name1 name2 = 0)
                                                && (loc1.line = loc2.line)
                                                && (loc1.byte = loc2.byte)
                                                && (String.compare loc1.file loc2.file = 0)
                                                && (String.compare typ1 typ2 = 0)
                                                && (id1 = id2) then delete_elem (name1, loc1, typ1, id1) xs
...
match list1 with x::xs -> (and_one_elem x list2)@(and_two_lists xs list2)
            | [] -> []

let rec and_several_lists list_of_lists =
match list_of_lists with x::y::xs -> and_several_lists ((and_two_lists x y)::xs)
                    | x::[] -> x
                    | [] -> []
